...
router.post('/chat', authMiddleware,rateLimiter, async (req, res) => {
  try {
    const { message, conversationId } = req.body; // ← now also expecting conversationId
...
const messages = contextText
  ...
  : [
    ...
    let fullResponse = ''; // ← accumulate the complete response as it streams

    for await (const chunk of stream) {
      const content = chunk.choices[0]?.delta?.content || '';
      if (content) {
        fullResponse += content; // ← build up the full text
